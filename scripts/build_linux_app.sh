...
#
# Configure + build ipcbench for native Linux x86_64 using repo-local linux_stage deps.
# Auto-loads generated/*/idl_gen.env (if present) and passes:
#   -DIDL_FILE=...
...
#
# Overrides:
#   BUILD_DIR=build-linux-debug BUILD_TYPE=Debug ./scripts/build_linux_app.sh
#   IPC_USE_FASTDDS=ON IPC_USE_PPS=OFF ./scripts/build_linux_app.sh
#   FD_SHIM_USE_WAITSET=OFF ./scripts/build_linux_app.sh
#   IDL_FILE=src/idl/Hello.idl ./scripts/build_linux_app.sh
#   IDL_ENV_FILE=generated/Hello/idl_gen.env ./scripts/build_linux_app.sh
#   LINUX_STAGE_OVERRIDE=/abs/path ./scripts/build_linux_app.sh
#
set -euo pipefail
...
LINUX_STAGE_DEFAULT="${ROOT_DIR}/linux_stage"
: "${LINUX_STAGE_OVERRIDE:=${LINUX_STAGE_DEFAULT}}"
LINUX_STAGE="${LINUX_STAGE_OVERRIDE}"

# Package config locations in stage prefix (aligned with QNX style)
FASTDDS_DIR="${LINUX_STAGE}/share/fastdds/cmake"
FASTCDR_DIR="${LINUX_STAGE}/lib/cmake/fastcdr"
FOONATHAN_DIR="${LINUX_STAGE}/lib/foonathan_memory/cmake"

log() { printf "\n\033[1;34m==> %s\033[0m\n" "$*"; }
die() { printf "\n\033[1;31mERROR:\033[0m %s\n" "$*" >&2; exit 1; }

[[ -d "${LINUX_STAGE}" ]] || die "Missing linux_stage: ${LINUX_STAGE} (run scripts/build_linux_deps.sh first)"
...
log "Configuring Linux app build"
log "  BUILD_DIR=${BUILD_DIR}"
log "  LINUX_STAGE=${LINUX_STAGE}"
